...
# Take smooth derivative
def discreteDerivative(f, x):
    dx = x[1] - x[0] # use np.diff(x) if x is not uniform
    gf = ndimage.gaussian_filter1d(f, sigma=1.5, order=1, mode='nearest') / dx
    return gf
...
class ForcingFunction:
    ...
    def train(self,attractor,example,time):
        ...
        exampleVel = discreteDerivative(example,time)
        exampleAccel = discreteDerivative(exampleVel,time)
...
#        exampleAccel=np.append(exampleAccel,[exampleAccel[-1],exampleAccel[-1]])
        ...
        for basisFunction in self.basisFunctions:
            ...
            P = np.mat(P)
            F = np.mat(F)
            self.weights[basisFunctionNumber]=np.transpose(S)*P*np.transpose(F)/(np.transpose(S)*P*S+.000000000000001)
